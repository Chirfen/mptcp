...
//scaling first,then multiply back
//the purpose of scaling numerator and denominator is to get a integer result after divide operation
...
//added by zhuo
static int mptcp_cal_k(struct sock *sk)
{
	int bs;
	int basertt_us=20000;
	struct tcp_sock *sub_tp = tcp_sk(sk);
	bs=sub_tp->srtt_us/basertt_us;
	return bs*(1+bs)/2;
}
...
static void mptcp_ccc_cong_avoid(struct sock *sk, u32 ack, u32 acked)
{
	...
	int snd_cwnd;
	//printk(KERN_INFO "zhuo modified!");
	...
	if (mpcb->cnt_established > 1) {
		...
		snd_cwnd = (int) div_u64 ((u64) mptcp_ccc_scale(1, alpha_scale),
						alpha);
		snd_cwnd=(snd_cwnd<<7)/(ca->rho2_7ls);
		
		/* snd_cwnd_cnt >= max (scale * tot_cwnd / alpha, cwnd)
		 * Thus, we select here the max value.
		 */
		if (snd_cwnd < (tp->snd_cwnd<<7)/(ca->rho2_7ls))
...
MODULE_LICENSE("GPL");
MODULE_DESCRIPTION("MPTCP fast convergence LINKED INCREASE CONGESTION CONTROL ALGORITHM");
MODULE_VERSION("0.1");
